...
RBEC::RBEC(const std::string& file) :
    mesh_file(file), beta(200.0), t(0.0), dt(1.0e-3), gamma_x(1.0), gamma_y(1.0), omega(0.0)
{};
...
void RBEC::initialValue()
{
    ...
    L2Phi_0 = Functional::L2Norm(phi_re, 6);
    std::cout << "L2Norm = " << L2Phi_0 << std::endl;
...
//////
	...
	{
	    double Jxw = quad_info.weight(l) * jacobian[l] * volume;
	    double phi2 = phi_re_value[l] * phi_re_value[l];
//////////Energy of Lz
	    e += Jxw * (0.5 * (phi_re_gradient[l][0] * phi_re_gradient[l][0] +phi_re_gradient[l][1] * phi_re_gradient[l][1]) 
